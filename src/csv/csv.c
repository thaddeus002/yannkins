...
/* ne prend que les champs correspondant à la condition min<=elementsCherches[elementCondition-1]<=max */
ligne_csv_t *lecture_fichier_csv_conditionnelle(char *nomFichier, char **elementsCherches, int nbElementsCherches, int elementCondition, float min, float max){
	FILE *fichier; /* le fichier à lire */
	int *colonnes; /* les colonnes de la table à utiliser, les colonnes sont numérotées à partir de 1 */
	int n; /* nb de colonnes trouvées */
	char ligne[TAILLE_BUF]; /* une ligne du fichier */
	int i,j,k,l; /*compteurs */
	ligne_csv_t *table; /* la valeur à renvoyer */
	int ajoutElement; /* 0 s'il ne faut pas ajouter le dernier element lu à la ligne_csv_t */
	float valeurElementCondition; /* valeur de l'élement de sélection conditionnelle */
	...
	/* lecture de la premiere ligne */
	colonnes=malloc(nbElementsCherches * sizeof(int));
	n=0;
	fgets(ligne, TAILLE_BUF, fichier);
	if(ligne!=NULL) n=recherche_colonnes_(ligne, colonnes, elementsCherches, nbElementsCherches);
	#ifdef DEBUG
	printf("%d éléments trouvés\n", n);
	for(i=0; i<nbElementsCherches; i++)
		printf("    c[%d] = %d\n", i, colonnes[i]);
	#endif
	...
	while(fgets(ligne, TAILLE_BUF, fichier)!=NULL){
		char champ[TAILLE_BUF]; /* une partie de la ligne */
		ligne_csv_t *element; /* conversion de la ligne en element à rajouter à la liste */
		ligne_csv_t *dernier; /* le dernier element de la liste */

		element = malloc(sizeof(ligne_csv_t));
		element->next=NULL;
		element->valeurs=malloc(nbElementsCherches * sizeof(char *));
		for(i=0; i<nbElementsCherches; i++) element->valeurs[i]=NULL;

		/* Les éléments sont séparés par des ; */

		i=0; k=0; /* i indice de caractère sur la ligne, k numéro de champ */
		while((ligne[i]!='\0')&&(ligne[i]!='\n')){
			j=0;
			while((ligne[i]!='\0')&&(ligne[i]!='\n')){
				if(ligne[i]==';'){
					/*le champ se tremine au ';' sauf si on n'a pas fermé les guillemets */
					if(champ[0]!='"') break;
					if(champ[j-1]=='"') break;
				}
				if(ligne[i]!='\r') champ[j]=ligne[i];
				else champ[j]='\0';
				j++; i++;
			}
			champ[j]='\0'; k++;
			/* dans certains cas (\r\n entre des guillements) notre champ est sur plusieurs lignes */
			if( (champ[0]=='"') && (champ[strlen(champ)-1]!='"') && (ligne[i]=='\n') ){
				int OK; /* fin de champ atteinte */
				OK=0;
				j=strlen(champ);
				//printf("champ tronqué: %s\n", champ);
				while(!OK){
					if(fgets(ligne, TAILLE_BUF, fichier)==NULL) { champ[j]='\0'; break; }
					i=0;
					//printf("Ligne de taille %d : %s\n", strlen(ligne), ligne);
					while(i<strlen(ligne)){
						//printf("%c(%d) ", ligne[i], ligne[i]);
						champ[j]=ligne[i];
						if(ligne[i]=='"') {
							j++;
							i++;
				...
				//printf("trouvé champ: %s\n", champ);
			}

			if(ligne[i]==';') i++;

			/* il peut y avoir des guillemets autour du champs */
			supprime_guillemets(champ);
			...
			for(l=0; l<nbElementsCherches; l++){
				if(k==colonnes[l]){
					element->valeurs[l]=malloc(strlen(champ)+1);
					strcpy(element->valeurs[l], champ);
					break;
				}
			}
		}
		...
		ajoutElement=0;
		if(elementCondition==0) ajoutElement=1;
		else if((elementCondition>0) && (elementCondition<=nbElementsCherches)){
			if( element->valeurs[elementCondition-1] != NULL){
				valeurElementCondition=atof(element->valeurs[elementCondition-1]);
				if( (valeurElementCondition>=min) && (valeurElementCondition<=max) ) ajoutElement=1;
			}
		}

		/* rajout de l'élément à la liste */
		if(ajoutElement){
			if(table==NULL) table=element;
			else dernier->next=element;
			dernier=element;
		}
	}
...
ligne_csv_t *lecture_fichier_csv(char *nomFichier, char **elementsCherches, int nbElementsCherches){
	return(lecture_fichier_csv_conditionnelle(nomFichier, elementsCherches, nbElementsCherches, 0, 0,0));
}
